...
export default function NodePanel() {
  const { building, selectedNodeId, updateNode } = useEditorStore()
  ...
  if (!node) {
    return (
      <div className="panel-right" style={{ width: 220, padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Properties</div>
        <div style={{ color: 'var(--text-muted)', fontSize: 13, textAlign: 'center', marginTop: 40, lineHeight: 1.7 }}>
        ...
        <div style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.6 }}>
          <b style={{ color: 'var(--text-secondary)' }}>Shortcuts:</b><br />
  ...
  return (
    <div className="panel-right animate-fade-in" style={{ width: 220, padding: 14, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Node Properties</div>
        <span className="badge badge-primary" style={{ fontSize: 10 }}>{node.type}</span>
      </div>
      ...
      {(node.type === 'stair' || node.type === 'elevator') && (
        <div>
          <div className="label">Link ID (for cross-floor)</div>
          <input
            className="input"
            value={node.linkId ?? ''}
      ...
      <div style={{ fontSize: 11, color: 'var(--text-muted)', background: 'var(--bg-surface)', padding: 8, borderRadius: 6, lineHeight: 1.5 }}>
        Floor: <span style={{ color: 'var(--text-secondary)' }}>{floor?.name}</span>
      </div>
    </div>
  )
}
